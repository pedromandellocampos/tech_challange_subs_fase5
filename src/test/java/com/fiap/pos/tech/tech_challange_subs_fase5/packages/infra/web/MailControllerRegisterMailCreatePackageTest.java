...
class MailControllerRegisterMailCreatePackageTest {
  ...
  public void createAndLoginEmployee() throws Exception {
    ...
    try {
      ...
      mockMvc.perform(post("/api/v1/employees").content(objectMapper.writeValueAsString(employee))
  ...
  public void createAndLoginResident() throws Exception {
    ...
    try {
      ...
      mockMvc.perform(post("/api/v1/residents").content(objectMapper.writeValueAsString(resident))
        .contentType(MediaType.APPLICATION_JSON)).andExpect(status().isCreated());
    }
  ...
  @Test
  void registerMail_success() throws Exception {
    ...
    mockMvc.perform(post("/api/v1/mail")
        .contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(createMailDTO)).header("Authorization", employeeToken))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.unity").value("101"));
  }

  @Test
  void registerMail_unauthorized() throws Exception {
  ...
  @Test
  void registerMail_resident() throws Exception {
    ...
    mockMvc.perform(post("/api/v1/mail")
        .contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(createMailDTO)).header("Authorization", residentToken))
      .andExpect(status().isForbidden());
  }
